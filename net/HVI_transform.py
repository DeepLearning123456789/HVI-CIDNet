...
class RGB_HVI(nn.Module):
    def __init__(self):
        ...
        self.alpha_s = 1.3
        #self.this_k = 0
        # 修改缓存变量：原先存储标量，现在用来缓存当前 Batch 的 k_map Tensor
        self.current_k_map = None
    ...
    def PHVIT(self, img):
        ...
        # 从缓存中取出对应的 k_map,由于 H, V, I 在上面被压平为了 [B, H, W]，为了矩阵对齐，我们需要把 [B, 1, H, W] 的 k_map 也压平
        k_map = self.current_k_map.squeeze(1) 
        # 利用对应的像素级 K 值进行逆向解压
